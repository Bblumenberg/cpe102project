...
public class MinerNotFull extends Miner{
    ...
    public TwoTuple<List<Point>,boolean> minerToOre(WorldModel world, PositionedEntity ore){
        if(ore == null){
            return new TwoTuple<List<Point>,boolean>(new ArrayList<Point>(position), false);
        }
        orePt = ore.getPosition();
        if(position.adjacent(orePt)){
            this.resourceCount += 1;
//          actions.removeEntity(world, ore);
            return new TwoTuple<List<Point>,boolean>(new ArrayList<Point>(orePt), true);
        }
        else{
            Point newPt = nextPosition(world, orePt);
            return new TwoTuple<List<Point>,boolean>(new ArrayList<Point>(world.moveEntity(newPt)), false);
        }
    }
